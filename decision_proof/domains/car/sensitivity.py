"""Car decision sensitivity thresholds."""
...
def thresholds(ir: dict[str, Any]) -> dict[str, Any]:
    ...
    if value_of_time is not None and current_transport_cost is not None:
        break_even_incremental_cost = (
            known_time_saved * value_of_time + non_time_value - margin
        )
        break_even_monthly_car_cost = (
            current_transport_cost + break_even_incremental_cost
        )
    break_even_value_of_time = None
    if known_time_saved > 0 and incremental_cost is not None:
        break_even_value_of_time = (
            incremental_cost - non_time_value + margin
        ) / known_time_saved

    break_even_time_saved_hours = None
    if value_of_time and value_of_time > 0 and incremental_cost is not None:
        break_even_time_saved_hours = (
            incremental_cost - non_time_value + margin
        ) / value_of_time

    required_lifestyle_premium = None
    if value_of_time is not None and incremental_cost is not None:
        required_lifestyle_premium = max(
            0.0, incremental_cost - (known_time_saved * value_of_time) + margin
        )
