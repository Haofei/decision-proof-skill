...
from decision_proof.core.domain_runtime import DomainRuntimeError, derived_value_dependencies, domain_key, evaluate, guidance, thresholds, verify
...
def render_markdown(run: dict[str, Any]) -> str:
	...
	for proof_goal in proof_state.get("goals", []):
		lines.append(f"- {goal_mark(proof_goal.get('status', 'unknown'))}: `{proof_goal.get('claim')}` - {proof_goal.get('reason')}")
