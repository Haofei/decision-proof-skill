...
def changed_values(before: dict[str, Any], after: dict[str, Any]) -> dict[str, dict[str, Any]]:
...
def diff_runs(before: dict[str, Any], after: dict[str, Any]) -> dict[str, Any]:
	...
	for claim in sorted(set(before_goals) | set(after_goals)):
		...
		if old.get("status") != new.get("status") or old.get("reason") != new.get("reason"):
			goal_changes[claim] = {
	...
	return {
		...
		"variable_changes": changed_values(flatten_variables(before), flatten_variables(after)),
		"derived_value_changes": changed_values(before.get("derived_values", {}), after.get("derived_values", {})),
...
def render_markdown(diff: dict[str, Any]) -> str:
	...
	lines.extend(["", "## Verifier Change", "", f"- `{diff['verifier_change']['from']}` -> `{diff['verifier_change']['to']}`", ""])
	return "\n".join(lines)
