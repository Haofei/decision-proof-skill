...
from core.domain_shared import evidence_quality_from_records, goal, has_failed_goal, recommendation_status
...
def evaluate_option(ir: dict[str, Any], option: dict[str, Any]) -> dict[str, Any]:
    ...
    if emergency_fund_months_after is None:
        goals.append(goal("G1", "cash_safety", "open", "emergency fund after option is unknown", [f"{option_id}.emergency_fund_months_after"], severity="warning"))
    elif emergency_fund_months_after >= min_emergency:
        goals.append(goal("G1", "cash_safety", "closed", f"{emergency_fund_months_after:g} months >= {min_emergency:g} months", [f"{option_id}.emergency_fund_months_after"], severity="soft"))
    else:
        goals.append(goal("G1", "cash_safety", "failed", f"{emergency_fund_months_after:g} months < {min_emergency:g} months", [f"{option_id}.emergency_fund_months_after"], severity="hard"))
    ...
    if baseline:
        goals.append(goal("G2", "income_affordability", "closed", "baseline option has no additional car affordability constraint", [f"{option_id}.monthly_cost"], severity="soft"))
    elif monthly_cost is None or income is None:
        goals.append(goal("G2", "income_affordability", "open", "monthly cost or income is unknown", [f"{option_id}.monthly_cost", "monthly_after_tax_income"], severity="warning"))
    elif cost_income_ratio <= max_ratio:
        goals.append(goal("G2", "income_affordability", "closed", f"cost is {cost_income_ratio:.1%} of after-tax income", [f"{option_id}.monthly_cost", "monthly_after_tax_income"], severity="soft"))
    elif cost_income_ratio <= hard_max_ratio:
        goals.append(goal("G2", "income_affordability", "failed", f"cost is {cost_income_ratio:.1%}, above {max_ratio:.0%} pressure threshold", [f"{option_id}.monthly_cost", "monthly_after_tax_income"], severity="warning"))
    else:
        goals.append(goal("G2", "income_affordability", "failed", f"cost is {cost_income_ratio:.1%}, above {hard_max_ratio:.0%} hard ceiling", [f"{option_id}.monthly_cost", "monthly_after_tax_income"], severity="hard"))
    ...
    if baseline:
        goals.append(goal("G3", "utility_result", "closed", "baseline option is comparison anchor", [f"{option_id}.baseline"], severity="soft"))
        ...
        goals.append(goal("G3", "utility_result", "open", "net monthly value cannot be computed without unknown variables", deps, severity="warning"))
    elif net_monthly_value > margin:
        goals.append(goal("G3", "utility_result", "closed", f"net monthly value is ${net_monthly_value:.0f}", [f"{option_id}.monthly_cost", f"{option_id}.monthly_time_saved_hours", "value_of_time"], severity="soft"))
    else:
        premium = abs(net_monthly_value - margin)
        goals.append(goal("G3", "utility_result", "failed", f"needs about ${premium:.0f}/month more benefit to break even", [f"{option_id}.monthly_cost", f"{option_id}.monthly_time_saved_hours", "value_of_time"], severity="warning"))

    if stability_months is None:
        goals.append(goal("G4", "future_need_stability", "open", "future need stability is unknown", [f"{option_id}.expected_need_stability_months"], severity="warning"))
    elif stability_months >= 12:
        goals.append(goal("G4", "future_need_stability", "closed", "need appears stable for at least 12 months", [f"{option_id}.expected_need_stability_months"], severity="soft"))
    else:
        goals.append(goal("G4", "future_need_stability", "assumption", "need stability is short or uncertain", [f"{option_id}.expected_need_stability_months"], severity="warning"))
