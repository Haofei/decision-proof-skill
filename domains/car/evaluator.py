...
from core.domain_shared import evidence_quality_from_variables, goal, has_failed_goal, recommendation_status
...
def evaluate(ir: dict[str, Any]) -> dict[str, Any]:
    ...
    if emergency_fund is None:
        goals.append(goal("G1", "cash_safety", "open", "emergency fund months cannot be derived", ["emergency_fund_months_after", "emergency_fund_balance", "monthly_required_expenses"], severity="warning"))
    elif emergency_fund >= min_emergency:
        goals.append(goal("G1", "cash_safety", "closed", f"{emergency_fund:g} months >= {min_emergency:g} months", ["emergency_fund_months_after", "min_emergency_fund_months"], severity="soft"))
    else:
        goals.append(goal("G1", "cash_safety", "failed", f"{emergency_fund:g} months < {min_emergency:g} months", ["emergency_fund_months_after", "min_emergency_fund_months"], severity="hard"))

    if car_cost_income_ratio is None:
        goals.append(goal("G2", "income_affordability", "open", "monthly_after_tax_income is missing", ["monthly_car_cost", "monthly_after_tax_income"], severity="warning"))
    elif car_cost_income_ratio <= max_ratio:
        goals.append(goal("G2", "income_affordability", "closed", f"car cost is {car_cost_income_ratio:.1%} of after-tax income", ["monthly_car_cost", "monthly_after_tax_income", "max_car_cost_income_ratio"], severity="soft"))
    elif car_cost_income_ratio <= hard_max_ratio:
        goals.append(goal("G2", "income_affordability", "failed", f"car cost is {car_cost_income_ratio:.1%}, above {max_ratio:.0%} pressure threshold", ["monthly_car_cost", "monthly_after_tax_income", "max_car_cost_income_ratio"], severity="warning"))
    else:
        goals.append(goal("G2", "income_affordability", "failed", f"car cost is {car_cost_income_ratio:.1%}, above {hard_max_ratio:.0%} hard ceiling", ["monthly_car_cost", "monthly_after_tax_income", "hard_max_car_cost_income_ratio"], severity="hard"))
    ...
    if net_monthly_value is None:
        ...
        goals.append(goal("G3", "benefit_exceeds_incremental_cost", "open", "net monthly value cannot be computed without unknown variables", sorted(set(open_deps)), severity="warning"))
    elif net_monthly_value > margin:
        goals.append(goal("G3", "benefit_exceeds_incremental_cost", "closed", f"net monthly value is ${net_monthly_value:.0f}", ["monthly_time_value", "incremental_car_cost", "comfort_value_monthly", "optionality_value_monthly"], severity="soft"))
    else:
        premium = abs(net_monthly_value - margin)
        goals.append(goal("G3", "benefit_exceeds_incremental_cost", "failed", f"needs about ${premium:.0f}/month more comfort, optionality, or time value", ["monthly_time_value", "incremental_car_cost", "comfort_value_monthly", "optionality_value_monthly"], severity="warning"))

    if value(ir, "expected_need_stability_months", None) is None:
        goals.append(goal("G4", "future_need_stability", "open", "future work/location/commute stability is unknown", ["expected_need_stability_months"], severity="warning"))
    elif value(ir, "expected_need_stability_months", 0) >= 12:
        goals.append(goal("G4", "future_need_stability", "closed", "need appears stable for at least 12 months", ["expected_need_stability_months"], severity="soft"))
    else:
        goals.append(goal("G4", "future_need_stability", "assumption", "need stability is short or uncertain", ["expected_need_stability_months"], severity="warning"))
