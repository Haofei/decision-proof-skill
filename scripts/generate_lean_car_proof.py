#!/usr/bin/env python3
"""Generate and check a concrete Lean proof for a car Decision IR."""
...
def derive(ir: dict[str, Any]) -> dict[str, Any]:
    ...
    net_monthly_value = monthly_time_value + comfort_value + optionality_value - incremental_car_cost - margin
...
def render_lean(data: dict[str, Any]) -> str:
    ...
    return f"""-- Generated by decision-proof/scripts/generate_lean_car_proof.py
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate and check a Lean proof for car Decision IR.")
