...
def verify_run(run: dict[str, Any], *, expected_hash: str) -> dict[str, Any]:
    ...
    hard_failed = any(goal.get("status") == "failed" and goal.get("severity") == "hard" for goal in goals)
    ...
    if run.get("input_ir_hash") == expected_hash:
        ...
        failed.append({"id": "input_hash_matches_ir", "message": "input_ir_hash does not match the canonical hash of input_ir"})

    if not (hard_failed and recommendation_status in {"recommend", "lean_yes"}):
        passed.append("hard_fail_blocks_positive_recommendation")
    else:
        failed.append({"id": "hard_fail_blocks_positive_recommendation", "message": "a severity=hard failed goal cannot coexist with recommend/lean_yes"})

    if recommendation_status != "insufficient_evidence" or open_goals:
        passed.append("insufficient_evidence_requires_open_goal")
    else:
        failed.append({"id": "insufficient_evidence_requires_open_goal", "message": "insufficient_evidence requires at least one open proof goal"})

    derived_dependencies = run.get("derived_value_dependencies", {})
    missing_dependency_entries = [name for name in run.get("derived_values", {}) if not derived_dependencies.get(name)]
    if not missing_dependency_entries:
        passed.append("derived_values_have_dependencies")
    else:
        failed.append({"id": "derived_values_have_dependencies", "message": f"missing dependency mappings for: {', '.join(sorted(missing_dependency_entries))}"})
    ...
    if not dependency_failures:
        ...
        failed.append({"id": "unknown_variables_do_not_feed_numeric_outputs", "message": "; ".join(dependency_failures)})

    ranking = run.get("comparison", {}).get("ranking", [])
    options = run.get("comparison", {}).get("options", [])
    if ranking and options:
        option_status = {option.get("id"): option.get("status") for option in options if isinstance(option, dict)}
        ranking_ok = True
        last_rank = None
        ...
        if ranking_ok:
            ...
            failed.append({"id": "option_ranking_respects_status_order", "message": "option ranking places a lower-status option ahead of a higher-status one"})
    else:
        passed.append("option_ranking_respects_status_order")
