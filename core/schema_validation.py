...
def _validate_locally(instance: Any, schema: dict[str, Any], location: str, errors: list[str]) -> None:
    expected_type = schema.get("type")
    if isinstance(expected_type, list):
        if not any(_matches_type(instance, item) for item in expected_type):
            errors.append(f"{location}: {_type_label(instance)} is not of type {expected_type}")
            return
    ...
    if "const" in schema and instance != schema["const"]:
        errors.append(f"{location}: {instance!r} was expected to be {schema['const']!r}")

    if isinstance(instance, str):
        min_length = schema.get("minLength")
        if isinstance(min_length, int) and len(instance) < min_length:
            errors.append(f"{location}: string is shorter than minimum length {min_length}")
        pattern = schema.get("pattern")
        if isinstance(pattern, str) and re.match(pattern, instance) is None:
            errors.append(f"{location}: {instance!r} does not match pattern {pattern!r}")
    ...
    if isinstance(instance, (int, float)) and not isinstance(instance, bool):
        ...
        if isinstance(minimum, (int, float)) and instance < minimum:
            errors.append(f"{location}: {instance} is less than the minimum of {minimum}")
        if isinstance(maximum, (int, float)) and instance > maximum:
            errors.append(f"{location}: {instance} is greater than the maximum of {maximum}")
